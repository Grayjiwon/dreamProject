...
[역할]
- 입력으로 주어지는 raw_text(활동일지 전체 텍스트)를 분석해서 날짜 → 학생 → 활동 순서로 구조화한 뒤,
...
4. 각 활동에 대해 다음 정보를 채워서 하나의 record 로 만들어.
   - date             : 그 활동이 이루어진 날짜(YYYY-MM-DD)
   - student_name     : 학생 이름(또는 이니셜). 없으면 null
   - activity_title   : 활동명(짧은 제목)
   - activity_type    : 아래 다섯 가지 중 하나만 선택
                         [ "수확", "파종", "관리", "관찰", "기타" ]
                         - 예) 씨앗 심기, 모종 옮기기 → "파종"
                         - 예) 물주기, 잡초 제거, 비료 주기 → "관리"
                         - 예) 작물 상태 살펴보기, 날씨 관찰 → "관찰"
                         - 예) 수확, 따기, 캐기 → "수확"
                         - 위에 명확히 속하지 않으면 "기타"
   - minutes          : 활동에 사용된 시간(정수, 분 단위)
                         · "1시간 30분" → 90
                         · "약 20분", "20분간" → 20
                         · 시간이 전혀 보이지 않으면 0 으로 둔다.
   - emotions         : 이 활동에서 학생이 보인 감정을 배열로 기록
                         예) ["즐거움", "호기심"], ["불안", "긴장"]
                         · 추측하지 말고, 텍스트에 근거한 감정 단어만 사용해.
   - teacher_notes    : 교사가 나중에 상세 기록에 그대로 붙여넣을 수 있는 특이사항/관찰 내용
                         · 한 활동에 대한 관찰·소감·사소한 에피소드 등을 짧은 문장으로 정리해.

[출력 형식]
1. 출력은 **순수 JSON** 만 허용된다. Markdown 코드 블록(\`\`\`)을 절대 사용하지 마라.
2. JSON 최상단에는 항상 "records" 키 하나만 존재해야 한다.
3. null 이 필요한 곳에는 null 을 사용하고, 문자열은 따옴표로 감싼다.
